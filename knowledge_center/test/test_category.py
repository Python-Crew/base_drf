...
def test_one_category_exists_should_succeed(client) -> None:
    ...
    assert response_content.get("main_page_category") == test_category.main_page_category


@pytest.mark.django_db
def test_one_main_page_category_should_succeed(client)->None:
    test_main_page_category=KnowledgeCenterCategory.objects.create(title="Rock-2", main_page_category= True)
    ...
    assert response_content.get("main_page_category") == test_main_page_category.main_page_category
